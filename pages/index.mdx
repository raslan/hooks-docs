...
From v2.0 onwards, all use-timing hooks now return objects instead of arrays. While this makes it easier to pick just the property you need, it also means this is a major breaking change for previous users of the library. If you are using the library in a previous version, you should migrate to the new version by replacing the array syntax with object destructuring as shown in usage examples.

All delay inputs for all hooks are in milliseconds. If you need to work with large time-frames such as hours, days, weeks, months, years, etc, you should use a transformation library like date-fns or multiplication with native Date. Examples using this are shown in the demo and in the example directory of this repository.
